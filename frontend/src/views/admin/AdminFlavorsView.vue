...
async function remove(id:number){ if(!confirm('¿Eliminar sabor?')) return; try{ await deleteM.mutateAsync(id) } catch(e: unknown){ alert((e as {response?:{status?:number}})?.response?.status===403 ? '403 — Solo ADMIN' : 'Error al eliminar') } }
</script>
