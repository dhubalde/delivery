    ...
    <v-dialog v-model="dlg" max-width="520">
      <v-card :title="editing ? 'Editar producto' : 'Nuevo producto'">
        <v-card-text>
          ...
          <v-select v-if="form.product_type==='POTE'" v-model="form.pote_size" :items="['KG_1','KG_HALF','KG_QUARTER']" label="Tamaño pote *" :error-messages="fieldErr('pote_size')" density="compact" />
...
const form=reactive({ name:'', category_id: null as number|null, product_type:'POTE' as 'POTE'|'UNIT', pote_size:'KG_1' as 'KG_1'|'KG_HALF'|'KG_QUARTER'|null, min_flavors:3, max_flavors:4, price:'' })
const details=ref<Record<string,string>>({}); const formError=ref('')
const fieldErr=(k:string)=>details.value[k]??''
const poteHint=computed(()=> form.product_type==='POTE' ? hint(form.pote_size, form.product_type) : '')
...
function openCreate(){ editing.value=null; Object.assign(form,{ name:'', category_id:null, product_type:'POTE', pote_size:'KG_1', min_flavors:3, max_flavors:4, price:'' }); details.value={}; formError.value=''; dlg.value=true }
function openEdit(p: {id:number;name:string;category_id:number;product_type:'POTE'|'UNIT';pote_size:'KG_1'|'KG_HALF'|'KG_QUARTER'|null;min_flavors:number|null;max_flavors:number|null;price:string}){ editing.value=p.id; Object.assign(form,{ name:p.name, category_id:p.category_id, product_type:p.product_type, pote_size:p.pote_size, min_flavors:p.min_flavors??3, max_flavors:p.max_flavors??4, price:p.price }); details.value={}; formError.value=''; dlg.value=true }
function validatePote(): string|null{
  if(form.product_type==='UNIT'){ if(form.pote_size) return 'UNIT no permite pote_size'; return null }
  if(!form.category_id) return 'Categoría requerida (BR-CAT-01)'
  if(!form.pote_size) return 'Tamaño requerido para POTE'
  if(form.pote_size==='KG_QUARTER' && (form.min_flavors!==2 || form.max_flavors!==3)) return '1/4KG requiere 2-3 gustos'
  if(['KG_1','KG_HALF'].includes(form.pote_size!) && (form.min_flavors!==3 || form.max_flavors!==4)) return '1KG / 1/2KG requiere 3-4 gustos'
  return null
}
async function save(){
  ...
  const payload: Record<string,unknown>={ name:form.name, category_id:form.category_id, product_type:form.product_type, pote_size: form.product_type==='UNIT'?null:form.pote_size, min_flavors: form.product_type==='UNIT'?null:form.min_flavors, max_flavors: form.product_type==='UNIT'?null:form.max_flavors, price:form.price }
...
async function remove(id:number){ if(!confirm('¿Eliminar producto?')) return; try{ await deleteM.mutateAsync(id) } catch(e: unknown){ alert((e as {response?:{status?:number}})?.response?.status===403 ? '403 — Solo ADMIN' : 'Error al eliminar') } }
</script>
