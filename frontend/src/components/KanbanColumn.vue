  ...
  <v-card :style="{ borderTop: `4px solid ${color}` }" class="d-flex flex-column" style="min-height: 420px">
    ...
    <v-card-text class="flex-grow-1">
      <div v-if="isLoading" class="d-flex flex-column ga-2">
        <v-skeleton-loader v-for="i in 3" :key="i" type="card" height="80" />
      </div>
      ...
      <KanbanCard v-for="o in orders" :key="o.id" :order="o" />
    </v-card-text>
  </v-card>
</template>
<script setup lang="ts">
import KanbanCard from '@/components/KanbanCard.vue'
defineProps<{ title: string; color: string; orders: { id: number; code: string; state: string; fulfillment: string; cash_declared: boolean; total: string; payments: { method: string; status: string }[] }[]; isLoading: boolean; isError: boolean }>()
defineEmits<{ retry: [] }>()
</script>
